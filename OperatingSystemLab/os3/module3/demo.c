...
void *send_thread_1(void *arg)
{
	...
	/*
		...
		1.msgtyp=0时，读取队列中的第一条消息
2.msgtyp>0时，读取队列中消息类型为msgtyp的第一条消息，除非形参msgflg包含了MSG_EXCEPT旗标，这时将会读取队列中消息类型为不等于msgtyp的第一条消息
3.msgtyp<0时，读取队列中消息类型≤|msgtyp|，且消息类型值最小的第一条消息
	*/
    ...
    while (1) {
        ...
        if (r_msg.mdir == send_1_to_recv){// 根据消息走向判断来源
            ...
            if (strcmp(r_msg.mtext, "end2") == 0) {
                ...
                msgsnd(msgid, &r_msg, sizeof(struct msgbuf), 0);
                printf("<Receive_thread> receive 'end2' from <Sender2>, returning 'over2'...");
